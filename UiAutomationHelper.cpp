...
UiAutomationHelper::UiAutomationHelper() : m_pAutomation(nullptr) 
{
    ...
    CoCreateInstance(CLSID_CUIAutomation, nullptr, CLSCTX_INPROC_SERVER, IID_IUIAutomation, (void**)&m_pAutomation);
}
...
QVector<ControlInfo> UiAutomationHelper::getAllWindowsTree() 
{
    ...
    if (pRoot) 
    {
        // 把root作为一个虚拟父节点，抓取它的所有子节点（即所有窗口）
...
QVector<NameInfo> UiAutomationHelper::getAllNameTree()
{
    QVector<NameInfo> result;
    if (!m_pAutomation)
    {
        return result;
    }
    ...
    if (pRoot)
    {
        ...
        desktopRoot.name = "桌面";
        desktopRoot.localizedControlType = "窗格";
...
QVector<ControlInfo> UiAutomationHelper::getControlsByWindowName(const QString& windowTitle) 
{
    ...
    if (pFound)
    {
        ...
        pFound->get_CurrentLocalizedControlType(&typeName);   
        pFound->get_CurrentHelpText(&helpText);  
        ...
        // 递归获取该窗口下的所有子孙节点，存入 windowRoot.children 中
...
void UiAutomationHelper::getChildrenRecursive(IUIAutomationElement* element, ControlInfo& parentInfo) 
{
    ...
    m_pAutomation->CreateTrueCondition(&pTrueCondition); 
    ...
    if (children) 
    {
        ...
        for (int i = 0; i < length; ++i)
        {
            ...
            if (child) 
            {
                ...
                child->get_CurrentLocalizedControlType(&typeName);   
                child->get_CurrentHelpText(&helpText);   
                child->get_CurrentIsEnabled(&bEnabled); 
...
void UiAutomationHelper::getChildrenName(IUIAutomationElement* element, NameInfo& parentInfo) 
{
    IUIAutomationElementArray* children = nullptr;
    IUIAutomationCondition* pTrueCondition = nullptr;
    m_pAutomation->CreateTrueCondition(&pTrueCondition); 
